  ...
  constructor() {
    ...
    anKan(tile) {
      for (let i = 0; i < this.card.length - 3; ++i) {
        if (utils.isSameTile(tile, this.card[i])) {
          if (utils.isSameTile(tile, this.card[i + 1]) && 
                utils.isSameTile(tile, this.card[i + 2]) && 
                utils.isSameTile(tile, this.card[i + 3])) {
    ...
    kaKan(tile) {
      for (let i = 0; i < this.ponCard.length; ++i) {
        if (utils.isSameTile(tile, this.ponCard[i][0])) {
          this.kanCard.push([tile, this.ponCard[i][0], this.ponCard[i][1], 
            this.ponCard[i][2], this.ponCard[i][3]]);
